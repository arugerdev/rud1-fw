...
func (p *Prober) Ping(ctx context.Context, target string) (*PingResult, error) {
	...
	if platform.SimulateHardware() {
		return simulatePing(target), nil
	}

	// iputils on Debian/RPi: -c count, -W per-packet timeout (s), -q summary only.
	cmd := exec.CommandContext(ctx, "ping", "-c", "3", "-W", "2", "-q", target)
	out, err := cmd.CombinedOutput()

	res := &PingResult{Target: target, Raw: truncateRaw(out)}
...
// eyeballing the UI). Reachable is always true in sim mode.
func simulatePing(target string) *PingResult {
	...
	return &PingResult{
		...
		RttMs:      rtt,
		PacketLoss: 0,
		Raw:        fmt.Sprintf("simulated=true target=%s rtt=%.3fms", target, rtt),
	}
}
